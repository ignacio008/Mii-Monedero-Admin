...
class _CenserItemState extends State<CenserItem> {
 ...
 List<ActivacionesTotal>activacionTotal=[];
    ...
    void getlistaActivacion(String idusuario)async{
   activacionTotal=await FetchData().getActivaciones(idusuario);
    print('Tengo activaciones ${activacionTotal.length} cards');
    print(idusuario);
    setState(() {
...
String generateRandomString(int len) {
 ...
 String generateRandomString2(int len) {
  var r = Random();
  const _chars = 'AaBbCcDdEeFfGgHhIiJjKkLlMmNnOoPpQqRrSsTtUuVvWwXxYyZz1234567890';
  id_variable2= List.generate(len, (index) => _chars[r.nextInt(_chars.length)]).join();
  return id_variable2;
}
  @override
  void initState() {
    ...
    getlistaActivacion(widget.censerModel.id);
  ...
  Widget build(BuildContext context) {
    ...
    getTopChanele(widget.censerModel.state);
         ...
         widget.adminmodel.typeAdmin!="superAdmin"? _showAlertoption(context): Navigator.pushReplacement(
          context,
          MaterialPageRoute(
              builder: (context) => DetailSuperAdmin (adminModel: widget.adminmodel, censerModel: widget.censerModel)
          )
      );
              ...
              Expanded(
                ...
                child: Column(
                  ...
                  children: <Widget>[
                    ...
                    Text(
                      widget.censerModel.suspended==false ? "Activo" : "Suspendido",
                      style: TextStyle(
                        color:  widget.censerModel.suspended==false ? Color.fromARGB(255, 24, 148, 206) : Colors.red,
                    ...
                    Text(
                        "Numero de unidad: " + widget.censerModel.numUnidad
                    ),
                    Text(
          ...
          onPressed: () {
            ...
            setState(() {
              widget.censerModel.suspended = false;
                ...
                QuerysService().UpdateSuspendedFalseCamion(idCenser: widget.censerModel.id);
            setState(() {
  ...
  void _showAlertoption(BuildContext context){
    ...
    print("La lista despues de ${stateCosto.length}");
        _activaciones(context,activacionTotal[0], stateCosto[0]);
          },
             padding: EdgeInsets.only(left:40,top:17, right:40, bottom:17),
            ...
            Navigator.of(context).pop();
             ...
             padding: EdgeInsets.only(left:75,top:17, right:75, bottom:17),
              child: Text("${widget.censerModel.suspended==false?"Suspender":"Activar"}",),
  ...
  void _activaciones(BuildContext context,ActivacionesTotal iconmodelActiva, StateCosto statecosto) async{
    //          setSpinnerStatus(true);
    print(iconmodelActiva.numeroActivacion);
    ...
    print(widget.censerModel.id);
        ...
        if(widget.adminmodel.saldoTaquilla<=199){
           ...
           bool erroguardar=await QuerysService().SaveGeneralInfoEstadoPago(reference: FirebaseReferencias.REFERENCE_EstadoPagoCamionero, id:id_variable, collectionValues:{
            'activaciones':false,
     ...
     Toast.show("Ha ocurrido un error, por favor, intente de nuevo", context, duration: Toast.LENGTH_LONG);
        ...
        bool erroguardarActivaciones=await QuerysService().UpdateInfoActivaciones(reference: FirebaseReferencias.REFERENCE_Activacions, id:widget.censerModel.id, collectionValues:{
            'fechaNuevaVenta': now,
             'numeroActivacion':iconmodelActiva.numeroActivacion+12,
          });

        if(erroguardarActivaciones){
          Toast.show("Ha ocurrido un error, por favor, intente de nuevo", context, duration: Toast.LENGTH_LONG);
        }else{

           bool erroguardarActivacionesCenser=await QuerysService().UpdateInfoActivacionesCenser(reference: FirebaseReferencias.REFERENCE_CENSERS, id:widget.censerModel.id, collectionValues:{
          ...
          bool erroguardarSadoActivaciones=await QuerysService().UpdateInfoSaldoTaquilla(reference: FirebaseReferencias.REFERENCE_ADMIN, id:widget.adminmodel.id, collectionValues:{
           'saldoTaquilla':widget.adminmodel.saldoTaquilla-statecosto.costo,
          });
          if(erroguardarSadoActivaciones){
            ...
            Toast.show("¡Se ha efectuado el pago con exito! Por favor de Reiniciar la aplicacion", context, duration: Toast.LENGTH_LONG);
